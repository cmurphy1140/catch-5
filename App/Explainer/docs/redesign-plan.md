...
After this pass Connor chose a golden oak table with the grain running across the screen over the felt, full-width taller auction pills, larger text throughout, and the iPhone 16 as the only device to verify against (D35). The SE and accessibility cells above describe the layout's fallback behaviour and are not re-checked per release.
