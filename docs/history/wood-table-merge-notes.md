...
| `Sources/CatchFiveUI/HandFanView.swift` | `handTransition(index:width:)` (discard removal while choosing trump, deal-in insertion at the start of play), `nonisolated static dealOrigin(index:count:width:)` (nonisolated because CI's Swift 6.1 treats view statics as main-actor; the local Xcode 26 does not), `MatchedCard`, `ShakeEffect` uses `Theme.Motion.shakeAmplitude`; label reads "YOUR HAND · DEALER" when you deal; frame height uses the standard card | No overlap expected |
| `Sources/CatchFiveUI/Tutorial/TutorialView.swift` | Background `WoodGrainView().ignoresSafeArea()`; panels and pills use `Theme.Wood.inlay` fills; `SeatTile` labels are one line each with `minimumScaleFactor(0.7)` and the tile takes `maxWidth: .infinity` so three share a row; lesson paragraphs get `lineSpacing(2)` | Keep your `SeatTile` portrait, but put the `PortraitView` above the name in the VStack, not beside it: side by side the labels broke mid-word ("Haz el", "DEAL ER") at the boosted text size |
| `Sources/CatchFiveUI/Tutorial/Lessons/*.swift` | Every lesson's paragraphs and tactic were shortened on 2026-09-05 (same facts, fewer words); `BiddingLesson` tiles sit in an `HStack(spacing: 8)` without spacers; `TrumpLesson` refill ring is ivory and the copy says "ringed" | Take this side for the `LessonText` calls; re-apply your cast names on top |
