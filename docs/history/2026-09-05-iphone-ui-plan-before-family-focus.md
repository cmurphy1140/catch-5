> Historical snapshot preserved September 12, 2026 before the family-first plan update. This is not the active task list; its diagnoses and timing estimates describe an older code revision. Use [the active plan](../superpowers/plans/2026-09-05-iphone-16-ui-ux-gameplay.md).
...
**Current state:** design and roadmap complete; application code unchanged. **Files added by this task:** this plan only in the repository; a delivery copy outside the repo. The original brief is now preserved at `docs/history/catch5-redesign-brief.md` (relocated during the September 5 folder cleanup). **Blockers to future implementation:** none for initial pause/layout work; physical iPhone validation is needed before claiming tactile/accessibility quality. **Open product choices:** confirm-play default remains off; include 16e explicitly when finalizing device matrix; select optional gameplay work only after testing. **Recommendation:** approve Tasks 1–2 as the first implementation slice, then reassess screenshots and real-device play before cosmetic expansion.
