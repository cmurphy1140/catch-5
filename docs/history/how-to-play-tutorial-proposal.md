...
| `Sources/CatchFiveUI/Tutorial/TutorialModel.swift` | `@MainActor final class TutorialModel: ObservableObject`. `@Published var lesson: Int`, `@Published var completed: Set<Int>` (persisted in `UserDefaults` under `tutorial.completed`), per-lesson exercise state. |
...
| 4a · Tricks | West led K♦, spades trump. Tap a card you may play. | Hand `9♦ A♠ 7♣ 2♦ Q♥ 8♠`. Can be built as a real `Hand`: verify with `Hand.legalMoves`. | 9♦ or 2♦. After a correct tap, advance to 4b after ~2.5 s. |
...
| 5 · Scoring | Assign High, Low, Jack, Five, Game to Us or Them. | Us: `A♠ 3♠ 10♣ 7♥ 7♦ 9♣ 4♦ 8♠`. Them: `5♠ J♠ 10♥ Q♦ 2♣ 6♥ 9♦ 8♣`. They bid 4. Scores before: 10–5. | Us: High, Low. Them: Jack, Five, Game. Check with `scoreHand(captured:trump:bidder:)`. On all correct, show settlement 10→12, 5→12 via `settle`. |
...
The last three tests are the point of the spec: fixtures are checked against the engine so a rules change can never leave the tutorial teaching something false. Add this page to the reading-order table in `docs/learning-path.md` so it exports with the rest.
