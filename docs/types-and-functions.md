...
| `RulesView` | the rules sheet as a guided walk on the felt: six chapters (The table, Deal and bidding, Play, Scoring, 9 and out, Reading the table) in inlay panels with gold numerals, a chapter rail of chips that scrolls to a chapter and lights the one at the top, and a figure per chapter built from the table's pieces (four portraits round an oval with "first to 25", six card backs and the bid ladder, a four-card trick with a Following suit / Trumped toggle and the winner ringed, five tappable point tiles with the Game ledger, a nine-segment gold meter, the screen notes with icons); the verbatim rule text stays visible under every figure; reached from the tutorial's Full rules button | manual |
| `RulesFigures` | `RulesFigures.swift`: the numbers the rules sheet draws (point tiles and their meanings, the Game ledger, the bid ladder, the match target, two example tricks with their winners), each checked against the engine | `rulesFiguresMatchTheEngine` |
