...
| `TablePause` | `TablePause.swift`: the reasons play must wait (scene inactive, welcome card, any sheet, any dialog or alert, the reopened last trick, the draw for dealer) and one `isPaused` answer; UI state only, never an action | `tablePauseHoldsWhileAnyCoverRemains` |
