...
| `resumeContext` | one line for the welcome card while a match is in progress: hand number, both scores and the phase ("Hand 4 · Your team 6, their team 12 · trick 3"); nil when there is nothing to resume | `resumeContextDescribesTheSavedMatch` |
...
| `loadDefault(in:)` | loads game, settings and history from a directory; a game or history that cannot be decoded is set aside as `game-corrupt.json` or `history-corrupt.json` rather than overwritten, and the fresh game says so; a full match replays in well under 300 ms, so there is no loading state | `corruptHistoryIsSetAsideByLoadDefault`, `corruptGameIsSetAsideAndTheFreshGameSaysSo`, `restoringALongReplayIsFastEnoughToNeedNoLoadingState` |
...
| Name | Purpose |
|---|---|
| `Settings`, `SettingsStore` | `Settings.swift`: Codable preferences (including `difficulty`, `hasSeenRules`, `playerName`, `playerPortrait` and `hasSignedIn`) with defaults for missing keys, a one-time migration of the old West/Partner/East defaults to the cast's names, and `delay(leadingTrick:)` for the computer pause per play speed; the store reads and writes JSON atomically | `delayDependsOnPlaySpeedAndLeadPosition`, `settingsRoundTripThroughDiskAndTolerateMissingKeys`, `settingsRoundTripKeepsPlayerNameAndPortrait`, `settingsWithoutPlayerFieldsLoadsSignedOut`, `oldSettingsFileMigratesDefaultSeatNamesToCast` |
...
| `WelcomeCard` | the returning player's card over the dimmed table: portrait and name, the `resumeContext` line, Continue game (until the match is won), New match (confirms only if it would throw away bids or plays), Settings; reopened by the table's chevron | manual |
| `RootView` | the app's root: owns the model and the tutorial model, opens on login or on the table with the welcome card (`initialScreen(for:)`, the covered table hidden from VoiceOver and the card marked modal, the dim layer darker under Increase Contrast), crossfades between login, intro and table | `rootOpensOnLoginUntilSignedInThenOnTheTable` |
...
| `ScoreBarView` | the compact bar on its solid header band: back-to-menu chevron, title with the hand number, the gear menu (undo last action, settings, statistics, tutorial, new game), both scores; the scores open the scoreboard | manual |
| `TableSurface`, `SeatView`, `Suit.buttonTint` | the table: partner across the top, West and East at the sides (tiles sized by `TableLayout` so they never collide with the pile; portrait, gold ring on the seat to act, VoiceOver label from `seatSummary(for:)`, card-back stack with count, dealer and bidder badges), gold text at the top-left naming trump (suit in its own colour) and the contract once trump is set, with no pill behind it, the deck at the top-right, the pile in the middle with each card nudged toward its seat and the winner ringed, status line with the hint and the show or hide last-trick controls, commentary; during the auction the dealer context line and the bid grid or the suit pills (full-width, red suits tinted red, each named beneath its glyph with a keep-and-draw preview, greyed pills carrying their reason as an accessibility hint, 9 and out handing off to the confirmation) stand where the pile will be; one message line under the status (undo toast, the reason a tap was refused, hint or explanation, notice, your call, placeholder), the hand-end card as a scrollable overlay whose buttons stack when they do not fit; the whole surface scrolls only when its content cannot fit (accessibility text sizes, the SE at XXXL), reads as one VoiceOver container, leaves the accessibility tree under the finished-hand card, and carries the status line's focus target | manual (see the verification record in [redesign-plan.md](redesign-plan.md)) |
...
| `TableFeedback` | `TableFeedback.swift`: one haptic per accepted action; `cue(action:trickWinner:handEnded:matchWinner:)` picks the most consequential outcome (match won or lost, hand ended, trick won or lost, play, call) and each cue maps to its `SensoryFeedback`, with a win and a loss feeling different | `oneFeedbackCuePerActionWithTheOutcomeThatMattersMost` |
| `TablePause` | `TablePause.swift`: the reasons play must wait (scene inactive, welcome card, any sheet, any dialog or alert, the reopened last trick) and one `isPaused` answer; UI state only, never an action | `tablePauseHoldsWhileAnyCoverRemains` |
...
| `CardView` | a card face at a given width (48 in the tutorial, 60 in the hand, 56 on the pile) that scales with Dynamic Type, with a top-left corner index for fanned hands and rest / playable / dimmed / pile styles (under Increase Contrast a dimmed card keeps 80% opacity and gains a dashed edge); `Suit.glyph`, `Suit.ink`, `Card.label` helpers |
