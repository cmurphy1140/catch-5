...
| `LoginView` | first launch: name, face, Easy/Standard, one gold New match button and nothing else to tap; calls `signIn` | manual |
...
| `WelcomeCard` | the returning player's card over the dimmed table: portrait and name, Continue game (until the match is won), New match (confirms only if it would throw away bids or plays), Settings; reopened by the table's chevron | manual |
