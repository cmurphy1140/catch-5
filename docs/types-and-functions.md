...
| `lastHumanAction`, `describe(_ action:)` | the human's latest accepted action and its toast wording ("9♣ played", "Bid 3"); cleared by undo and new hands; also the trigger for the play and selection haptics | `lastHumanActionDescribesThePlayAndClearsOnUndo` |
...
| `Theme` | `Theme.swift`: card metrics (2:3 ratio, six-percent corner radius, hand 60/64 pt, pile 56 pt, overlap −16 pt leaving a 44 pt strip), motion springs and the trick hold, toast and shake timings | `fannedHandCardsKeepAThumbSizedStrip` |
| `ScoreBarView` | the compact bar: title and gear menu (settings, statistics, tutorial, new game), both scores, trump and contract in gold, hand number; the scores open the scoreboard | manual |
| `TableSurface`, `SeatView` | the table: partner across the top, West and East at the sides (gold ring on the seat to act, card-back stack with count, dealer and bidder badges), the pile in the middle with each card nudged toward its seat and the winner ringed, status line with the hint and last-trick controls, commentary, bid and trump controls during the auction, the hand-end card as an overlay | manual |
| `HandFanView`, `CardPressStyle`, `ShakeEffect` | the fanned hand: overlap, ±8° rotation and a shallow arc, legal cards lifted on your turn, press-lift, shake on an illegal tap, `matchedGeometryEffect` flight into the pile | manual |
| `CardBackView` | a face-down card for opponents' seats | manual |
| `TableView` | the root: score bar, table surface and hand in a non-scrolling column; one animation per accepted action; the scheduler task that holds a finished trick, collapses it toward the winner and then lets the next computer act; the undo toast; the haptic vocabulary; the sheets (text styles throughout, seat tiles and scores read as single VoiceOver elements, played cards announce seat and card, hand cards announce legality, Reduce Motion replaces flights with fades): header, scores and contract, three opponent tiles, status line naming who is thinking, discard notice, Hint and Undo buttons and advice panel on your turn (the suggested card is ringed in gold), trick area where tapping any played card explains it, a match-over card with the human's record, Review hand and Deal next hand buttons, hand, phase-specific controls, new-game confirmation, error alert, computer scheduler task, background save |
