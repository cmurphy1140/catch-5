...
| `TutorialView`, `SeatTile`, `DealLesson`, `BiddingLesson`, `TrumpLesson`, `TricksLesson`, `ScoringLesson` | the "How to play" sheet from the menu and the gear menu (seat tiles carry the cast's portraits): header, chapter pills, one lesson at a time, Back / Next, a Full rules button that opens `RulesView` | manual |
...
| `SettingsView` | sheet from the menu and the gear button: difficulty, play speed, your name and face, the three opponents' names beside their portraits, haptics toggle, and the "How Catch 5 is built" explainer | manual |
| `Theme` | `Theme.swift`: card metrics (2:3 ratio, six-percent corner radius, hand 60/64 pt, pile 56 pt, overlap −16 pt leaving a 44 pt strip, the 402 pt width that earns wide cards, the XXXL cap on card scaling), `Theme.Table` seat nudges and the pile footprint, the AX2 cap on table text, motion springs, the trick hold, toast and shake values | `fannedHandCardsKeepAThumbSizedStrip` |
| `ScoreBarView` | the compact bar: back-to-menu chevron, title and gear menu (undo last action, settings, statistics, tutorial, new game), both scores, trump and contract in gold, "YOU DEAL" in gold when you deal, hand number; the contract and hand lines stack at accessibility text sizes; the scores open the scoreboard | manual |
| `TableSurface`, `SeatView`, `Suit.buttonTint` | the table: partner across the top, West and East at the sides (portrait, gold ring on the seat to act, VoiceOver label from `seatSummary(for:)`, card-back stack with count, dealer and bidder badges), the pile in the middle with each card nudged toward its seat and the winner ringed, status line with the hint and last-trick controls, commentary; during the auction the bid grid or the suit buttons (red suits tinted red) stand where the pile will be; the hand-end card as a scrollable overlay whose buttons stack when they do not fit; the whole surface scrolls only when its content cannot fit (accessibility text sizes, the SE at XXXL) and reads as one VoiceOver container | manual (see the verification record in [redesign-plan.md](redesign-plan.md)) |
| `HandFanView`, `CardPressStyle`, `ShakeEffect`, `MatchedCard` | the fanned hand: overlap that tightens to fit the width, wide cards on screens of 402 pt and up, ±8° rotation and a shallow arc, legal cards lifted on your turn, press-lift, shake on an illegal tap, `matchedGeometryEffect` flight into the pile (`MatchedCard` drops the join under Reduce Motion so the card crossfades instead) | manual |
| `CardBackView` | a face-down card for opponents' seats | manual |
| `TableView` | the gameplay screen (shown by `RootView`, which it leaves through `onLeave`): score bar, table surface, toast slot and hand in a column; one animation per accepted action; the scheduler task that holds a finished trick, collapses it toward the winner and then lets the next computer act; the undo toast (your last action, any discard notice, and Undo) with "You: Bid 3" as the standing fallback during the auction; the haptic vocabulary; the sheets, the new-game confirmation, the error alert and the background save; text styles throughout, with cards capped at XXXL and table text at AX2 | manual |
| `TableScheduler.plan(hand:collapsedTricks:)` | the scheduler's two decisions as a pure function: hold a trick that has finished but not yet collapsed, and treat the coming computer play as a lead (longer pause) only when nothing was held | `schedulerHoldsAFinishedTrickOnceThenTreatsTheNextPlayAsALead` |
