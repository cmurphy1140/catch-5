...
## D35. A drawn oak table, larger text everywhere, and the iPhone 16 as the only verified device (PR #21, 2026-09-05)

**Chosen:** `WoodGrainView` draws the background in a `Canvas` from a fixed seed: a warm gradient, tonal bands, a few hundred wavy grain lines at uneven spacing (some swinging wide like figure in a board), and a vignette. The grain runs across the screen, like a board laid the long way under the phone, and the tone is a golden oak: Connor asked for both after seeing a first cut in vertical walnut. The oak stays mid-dark (about 6:1 against ivory) because the table's text sits straight on it; the grain and the vignette carry the lighter look. Seat tiles, panels and the hand-end card sit on a dark inlay colour instead of translucent white. Every text style renders two Dynamic Type steps above the system setting (`Theme.textBoostSteps`, applied at the root of `TableView` so sheets inherit it), which keeps the user's own size setting meaningful while making the whole app read larger. The bid, pass and suit pills fill their columns with 6 pt between them and stand 52 pt tall. The score bar sits on a solid band of the inlay colour that runs up behind the status bar and curves off at the bottom (28 pt continuous corners, a soft shadow), so the scores read on one colour and the wood starts beneath it. The bar carries only the title, the hand number and the scores: trump and the contract moved to a larger gold pill on the table under the header, and your dealer badge joined the "YOUR HAND" label, after Connor found the bar's bottom line too crowded. Connor asked for all of these, and for the layout to be judged on the iPhone 16 alone.

**Over:** A felt gradient (Connor: too generic); a paler oak, which would have pushed ivory and gold text below readable contrast; a wood image asset, which would need a resource pipeline the hand-built simulator bundle does not have; bumping each font style by hand across forty views; keeping the SE and accessibility-size matrix as a gate.
